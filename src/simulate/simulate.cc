...
void MakeControlSection(mj::Simulate* sim) {
  ...
  for (int i = 0; i < sim->actuator_ctrlrange_.size() && itemcnt < mjMAXUIITEM; i++) {
    ...
    if (sim->actuator_ctrlrange_[i].has_value())
      mju::sprintf_arr(
          defSlider[0].other, "%.4g %.4g", sim->actuator_ctrlrange_[i]->first, sim->actuator_ctrlrange_[i]->second);
    else {
      mju::strcpy_arr(defSlider[0].other, "-1 1");
    }
...
void UiEvent(mjuiState* state) {
  ...
  if (state->type == mjEVENT_MOVE && state->dragrect == 3 && (sim->m_ || sim->is_passive_)) {
    ...
    if (sim->pert.active) {
      mjv_movePerturb(model, data, action, state->dx / r.height, -state->dy / r.height, &sim->scn, &sim->pert);
    } else {
      mjv_moveCamera(model, action, state->dx / r.height, -state->dy / r.height, &sim->scn, &sim->cam);
    }
    return;
  }
...
Simulate::Simulate(
    std::unique_ptr<PlatformUIAdapter> platform_ui, mjvCamera* cam, mjvOption* opt, mjvPerturb* pert, bool is_passive)
